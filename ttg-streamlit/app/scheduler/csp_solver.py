...
class CSPSolver:
    ...
    def _build_neighbors(self) -> Dict[str, List[str]]:
        ...
        for i, v1 in enumerate(self.variables):
            for j, v2 in enumerate(self.variables):
                if i >= j:
                    continue
                if v1.faculty_id == v2.faculty_id or v1.section_id == v2.section_id:
    ...
    def _forward_check(self, var_key: str, value: Tuple[int, int], assignment: Assignment) -> Optional[Dict[str, List[Tuple[int, int]]]]:
        ts, room = value
        var = self.var_map[var_key]
